...
public class MiaoshaController implements InitializingBean{
    ...
    public void afterPropertiesSet() throws Exception{
        ...
        for(GoodsVo goods: goodsList){
            redisService.set(GoodsKey.getMiaoshaGoodsStrock,""+ goods.getId(),goods.getStockCount());
    ...
    public Result<Boolean> reset(Model model){
        ...
        for(GoodsVo goods : goodsList){
            ...
            redisService.set(GoodsKey.getMiaoshaGoodsStrock,""+goods.getId(),10);
            localOverMap.put(goods.getId(),false);
        }
    ...
    public Result<Integer> miaosha(Model model, MiaoshaUser user,
        ...
        //预减库存
        long stock = redisService.decr(GoodsKey.getMiaoshaGoodsStrock,""+goodsId);//10
        if(stock<0){
    ...
    @AccessLimit(seconds=5, maxCount=5, needLogin=true)   //限制访问次数，5秒钟访问5次
